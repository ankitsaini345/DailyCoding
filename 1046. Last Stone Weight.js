...
var lastStoneWeight = function (stones) {
  ...
  while (maxHeap.length) {
    const first = remove(maxHeap);
    if (!maxHeap.length) return first;
    const second = remove(maxHeap);
    if (first == second){
        if(!maxHeap.length) return 0;
        continue;
    }
    else insert(maxHeap, Math.abs(first - second));
  }
};
